...
void setProbabilities(double probabilities[NUM_CITIES], int ant,
                      int visited[NUM_ANTS][NUM_CITIES],
...
void debugPath(int path[NUM_CITIES]) {
  for (int i = 0; i < NUM_CITIES; i++) {
    printf("%d ", path[i]);
    if (i != NUM_CITIES - 1) {
      printf(" -> ");
...
void aco(int distances[NUM_CITIES][NUM_CITIES]) {
  ...
  for (int iterNum = 0; iterNum < NUM_ITERATIONS; iterNum++) {
    ...
    for (int ant = 0; ant < NUM_ANTS; ant++) {
      ...
      for (int move = 1; move < NUM_CITIES; move++) {
        ...
        setProbabilities(probabilities, ant, visited, pheromones, previousCity,
                         distances);
  ...
  // printPheromoneMatrix(pheromones);
  printf("Shortest: %d\n", shortestPathLength);
  debugPath(shortestPath);
}
